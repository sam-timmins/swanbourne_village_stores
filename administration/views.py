...
from orders.forms import CreateCollectionDayForm
...
def collection_days(request):
    ...
    return render(request, 'administration/collection-days.html', context)
...
def delete_collection_day(request, pk):
    ...
    """
    if not request.user.is_superuser:
        messages.info(request, 'Only store owners can delete a \
            collection day')
        return redirect(reverse('home'))
